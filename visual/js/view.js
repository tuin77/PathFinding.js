...
var View = {
  ...
  nodeStyle: {
    normal: {
      fill: "rgba(255, 255, 255, 0.1)", // 修改为半透明背景
      "stroke-opacity": 0.1, // the border
    },
    blocked: {
      fill: "rgba(128, 128, 128, 0.8)", // 修改为半透明背景
      "stroke-opacity": 0.2,
    },
    start: {
      fill: "rgba(5, 252, 5, 1)",
  ...
  generateGrid: function (callback) {
    ...
    createRowTask = function (rowId) {
      return function (done) {
        ...
        $stats.text(
          "generating grid " + Math.round(((rowId + 1) / numRows) * 100) + "%"
        );
        done(null);
      };
    };
